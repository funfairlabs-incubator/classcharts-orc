...
const FLOWS: FlowStep[] = [
  ...
  { from: 'browser',      to: 'fcm',           label: 'Register token', detail: 'On first visit to /settings, browser registers firebase-messaging-sw.js service worker, calls getToken() with VAPID key, POSTs the FCM token to /api/fcm-token which stores it in GCS user-prefs.json.' },
  { from: 'fcm',          to: 'browser',       label: 'Push notify',    detail: 'FCM delivers web push to the registered service worker. Foreground messages handled by the Firebase SDK; background messages handled by firebase-messaging-sw.js which calls showNotification(). Tapping opens the relevant deep-link.' },
  ...
  { from: 'frontend',     to: 'firestore',     label: 'Archive + status', detail: 'Reads archived announcements, attachment metadata, and poller heartbeat for status page' },
